...
class Shared_key_tools:
    ...
    def encrypt(self):
        ...
        self._encrypted_data = self._fernet.encrypt(bytes(data_to_encrypt))
        print ("encryoted data:",self._encrypted_data)
        self.decrypt()
        
    def decrypt(self):
        data = self._fernet.decrypt(bytes(self._encrypted_data))
